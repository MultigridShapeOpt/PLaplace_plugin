...
////////////////////////////////////////////////////////////////////////////////
		...
		void PLaplaceDerivativeRHS<TDomain>::
		...
		//	check number
			if(vLfeID.size() != (size_t)dim)
				UG_THROW("PLaplaceDerivativeRHS: Needs exactly "<<dim<<" functions.");

			//	check & set order
			for(size_t i = 0; i < vLfeID.size(); ++i)
				if(vLfeID[i].order() < 1)
					UG_THROW("PLaplaceDerivativeRHS: Adaptive order not implemented.");
		...
		prep_elem_loop(const ReferenceObjectID roid, const int si)
		{
			...
			}UG_CATCH_THROW("PLaplaceDerivativeRHS::prep_elem_loop:"
		...
		template<typename TDomain>
		template<typename TElem, typename TFEGeom>
		void PLaplaceDerivativeRHS<TDomain>::fsh_elem_loop()
		{}
		...
		prep_elem(const LocalVector& u, GridObject* elem, const ReferenceObjectID roid, const MathVector<dim> vCornerCoords[])
		{
			...
			UG_CATCH_THROW("PLaplaceDerivativeRHS::prep_elem:"
							" Cannot update Finite Element Geometry.");
		...
		add_rhs_elem(LocalVector& d, GridObject* elem, const MathVector<dim> vCornerCoords[])
		{
			...
			//const number c_new = pow(m_control, laplacian_power_1);
		...
		template<typename TDomain>
		PLaplaceDerivativeRHS<TDomain>::
		PLaplaceDerivativeRHS(const char* functions, const char* subsets)
		...
		//	set defaults
			...
			m_bOnlySensitivity = false;
			m_step_length = 1.0;
			m_control = 1.0;
		//	check number of functions
			if(this->num_fct() != dim)
				UG_THROW("Wrong number of functions: The ElemDisc 'PLaplaceDerivativeRHS'"
		...
		template<typename TDomain>
		PLaplaceDerivativeRHS<TDomain>::
		PLaplaceDerivativeRHS(const std::vector<std::string>& vFct,
		...
		#ifdef UG_DIM_1
		template<>
		void PLaplaceDerivativeRHS<Domain1d>::register_all_funcs(int order,
				int quadOrder)
		{
			//	RegularEdge
		...
		#ifdef UG_DIM_2
		template<>
		void PLaplaceDerivativeRHS<Domain2d>::register_all_funcs(int order,
				int quadOrder)
		{
		...
		#ifdef UG_DIM_3
		template<>
		void PLaplaceDerivativeRHS<Domain3d>::register_all_funcs(int order,
				int quadOrder)
		{
			if (quadOrder != 2 * order + 1) {
		...
		template<typename TDomain>
		template<typename TElem, typename TFEGeom>
		void PLaplaceDerivativeRHS<TDomain>::register_fe_func()
		{
		...
		#ifdef UG_DIM_2
		template class PLaplaceDerivativeRHS<Domain2d> ;
		#endif
		#ifdef UG_DIM_3
		template class PLaplaceDerivativeRHS<Domain3d> ;
		#endif
