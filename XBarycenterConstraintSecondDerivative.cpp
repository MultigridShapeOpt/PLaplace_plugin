...
////////////////////////////////////////////////////////////////////////////////
		...
		void XBarycenterConstraintSecondDerivative<TDomain>::
		...
		//	check number
			if(vLfeID.size() != (size_t)dim)
				UG_THROW("XBarycenterConstraintSecondDerivative: Needs exactly "<<dim<<" functions.");

			//	check & set order
			for(size_t i = 0; i < vLfeID.size(); ++i)
				if(vLfeID[i].order() < 1)
					UG_THROW("XBarycenterConstraintSecondDerivative: Adaptive order not implemented.");
		...
		template <typename TDomain>
		void
		XBarycenterConstraintSecondDerivative<TDomain>::
		set_assemble_funcs()
		{
		...
		prep_elem_loop(const ReferenceObjectID roid, const int si)
		{
			...
			}UG_CATCH_THROW("XBarycenterConstraintSecondDerivative::prep_elem_loop:"
		...
		template<typename TDomain>
		template<typename TElem, typename TFEGeom>
		void XBarycenterConstraintSecondDerivative<TDomain>::fsh_elem_loop()
		{}
		...
		prep_elem(const LocalVector& u, GridObject* elem, const ReferenceObjectID roid, const MathVector<dim> vCornerCoords[])
		{
			...
			UG_CATCH_THROW("XBarycenterConstraintSecondDerivative::prep_elem:"
							" Cannot update Finite Element Geometry.");
		...
		add_rhs_elem(LocalVector& d, GridObject* elem, const MathVector<dim> vCornerCoords[])
		{
			...
			for (size_t ip = 0; ip < geo.num_ip(); ++ip){
				...
				for (size_t sh1 = 0; sh1 < geo.num_sh(); ++sh1){
					...
					d(d1,sh1) +=  m_multiplier * vCurrentPosition[m_index]*TrTransformedDdeltau* geo.weight(ip)*dDF; //<Test_lb,(x+u)>Tr(InvDF*Ddu)*detDF
		...
		template<typename TDomain>
		XBarycenterConstraintSecondDerivative<TDomain>::
		XBarycenterConstraintSecondDerivative(const char* functions, const char* subsets)
		...
		//	check number of functions
			if(this->num_fct() != dim)
				UG_THROW("Wrong number of functions: The ElemDisc 'XBarycenterConstraintSecondDerivative'"
		...
		template<typename TDomain>
		XBarycenterConstraintSecondDerivative<TDomain>::
		XBarycenterConstraintSecondDerivative(const std::vector<std::string>& vFct,
		...
		#ifdef UG_DIM_1
		template<>
		void XBarycenterConstraintSecondDerivative<Domain1d>::register_all_funcs(int order,
				int quadOrder)
		{
			//	RegularEdge
		...
		#ifdef UG_DIM_2
		template<>
		void XBarycenterConstraintSecondDerivative<Domain2d>::register_all_funcs(int order,
				int quadOrder)
		{
		...
		#ifdef UG_DIM_3
		template<>
		void XBarycenterConstraintSecondDerivative<Domain3d>::register_all_funcs(int order,
				int quadOrder)
		{
			if (quadOrder != 2 * order + 1) {
		...
		template<typename TDomain>
		template<typename TElem, typename TFEGeom>
		void XBarycenterConstraintSecondDerivative<TDomain>::register_fe_func()
		{
		...
		#ifdef UG_DIM_2
		template class XBarycenterConstraintSecondDerivative<Domain2d> ;
		#endif
		#ifdef UG_DIM_3
		template class XBarycenterConstraintSecondDerivative<Domain3d> ;
		#endif
