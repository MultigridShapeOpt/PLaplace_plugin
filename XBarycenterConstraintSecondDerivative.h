...
#ifndef XBARYCENTER_SECOND_DERIVATIVE
#define XBARYCENTER_SECOND_DERIVATIVE
...
namespace ug{
	namespace PLaplacian{
		...
		class XBarycenterConstraintSecondDerivative: public IElemDisc<TDomain>
		{
			//CONSTRUCTOR's
			public:
				XBarycenterConstraintSecondDerivative(const char* functions, const char* subsets);
				XBarycenterConstraintSecondDerivative(const std::vector<std::string>& vFct, const std::vector<std::string>& vSubset);
			...
			typedef XBarycenterConstraintSecondDerivative<TDomain> this_type;
			//PUBLIC MEMBERS:
			public:
				///	quadrature order
			...
			public:
				...
				void set_index(size_t ind){//if Bx the ind = 1, if By ind = 2 (LUA INDEXED, starts at 1)
					this->m_index=ind-1.0;
				}
		...
		};//end XBarycenterConstraintSecondDerivative
