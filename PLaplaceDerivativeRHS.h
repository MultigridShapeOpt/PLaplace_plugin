...
#ifndef P_LAPLACE_DERIVATIVE_RHS
#define P_LAPLACE_DERIVATIVE_RHS
...
namespace ug{
	namespace PLaplacian{
		...
		class PLaplaceDerivativeRHS: public IElemDisc<TDomain>
		{
			//CONSTRUCTOR's
			public:
				PLaplaceDerivativeRHS(const char* functions, const char* subsets);
				PLaplaceDerivativeRHS(const std::vector<std::string>& vFct, const std::vector<std::string>& vSubset);
			...
			typedef PLaplaceDerivativeRHS<TDomain> this_type;
			//PUBLIC MEMBERS:
			public:
				///	quadrature order
			...
			protected:
				...
				bool m_bOnlySensitivity;

			public:
				void set_flag_sensitivity(bool f)
				{
					this->m_bOnlySensitivity = f;
				}
